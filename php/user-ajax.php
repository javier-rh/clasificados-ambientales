...
function ModelGetStateByCountryID($con,$config)
{
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if($count == 1)
            {
                $list .=  '<li class="selected"><a class="selectme" data-id="'.$country_id.'" data-name="'.$lang['ALL'].' '.$countryName.'" data-type="'.$lang['COUNTRY'].'"><strong>'.$lang['ALL'].' '.$countryName.'</strong></a></li>';
            }
        ...
        if ($total > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if($count == 1)
                {
                    ...
                    $list .=  '<li class="selected"><a class="selectme" data-id="'.$state_id.'" data-name="'.$stateName.', '.$lang['REGION'].'" data-type="'.$lang['REGION'].'"><strong>'.$lang['WHOLE'].' '.$stateName.'</strong></a></li>';
                }

                $list .= '<li class=""><a id="region'.$id.'" class="selectme" data-id="'.$id.'" data-name="'.$name.', '.$lang['CITY'].'" data-type="'.$lang['CITY'].'"><span>'.$name.' <i class="fa fa-angle-right"></i></span></a></li>';
                $count++;
            }
 ...
 LIMIT 20";
    ...
    if ($total > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $list .= '<li><a href="#" class="title selectme" data-id="'.$cityid.'" data-name="'.$cityname.'" data-type="'.$lang['CITY'].'">'.$cityname.', <span class="color-9">'.$statename.'</span></a></li>';
        }
        $list .= '</ul>';
        echo $list;
    }
...
function hideItem($con,$config)
{
    ...
    if (trim($id) != '') {
        ...
        $status = $info['hide'];
        if($status == "0"){
            $con->query("UPDATE `".$config['db']['pre']."product` set hide='1' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ");
            echo 1;
        }else{
...
LEFT JOIN `".$config['db']['pre']."usergroups` as g ON g.group_id = u.group_id
 WHERE p.product_name like '%$qString%' and p.status = 'active' and p.country = '".$country_code."' and g.show_in_home_search = 'yes'
 ORDER BY
  CASE
