jQuery(function($) {
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    ...
                    if(response != 0) {
                        $item.remove();
                        hideSiteActionBtn();
                        alertify.success("¡Eliminado! el registro ha sido eliminado con éxito");
                    }else{
                        alertify.error("Hubo un problema al eliminarlo, intentalo una vez mas.");
                    }
    ...
    $('#ajax-services').on('click', '.item-approve', function(e) {
        ...
        swal({
            ...
            $.post(ajaxurl+'?action='+action, data, function(response) {
                    if(response != 0) {
                        $item.find('.label').html("Aprobado");
                        $item.find('.label').removeClass('label-warning');
                        $item.find('.label').addClass('label-success');
                        $item.find('.item-approve').remove();

                        alertify.success("¡Aprobado! El registro ha sifo aprobado.");
                        swal.close();
                    }else{
                        swal("¡Error!", "Hubo un problema al aprobarlo, intentalo una vez mas.", "error");
                    }
                    jQuery('.confirm').removeClass('bookme-progress');
    ...
    $('#ajax-services').on('click', '.user-js-active', function(e) {
        ...
        swal({
            title: "¿Estás seguro?",
            text: "¿Quieres activar éste usuario?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#8BC34A",
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    if(response != 0) {
                        $item.find('.label').html("ACTIVADO");
                        $item.find('.label').removeClass('label-warning');
                        $item.find('.label').addClass('label-info');
                        $item.find('.user-js-active').remove();
                        alertify.success("¡Activado! El usuario ha sido activado.");
                        swal.close();
                    }else{
                        swal("¡Error!", "Hubo un problema al activar, intentalo una vez mas.", "error");
                    }
                    jQuery('.confirm').removeClass('bookme-progress');
                },
                error: function (response, textStatus, errorThrown) {
                    swal("Error!", "Hubo un problema al activarlo, intentalo una vez mas.", "error");
                    jQuery('.confirm').removeClass('bookme-progress');
                }
            });
        });
    ...
    $('#ajax-services').on('click', '.user-js-ban', function(e) {
        ...
        swal({
            title: "¿Estás seguro?",
            text: "¿Quieres bloquear a éste usuario?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#f44336",
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    ...
                    success: function (response, textStatus, jqXHR) {
                        if(response != 0) {
                            $item.find('.label').html("BLOQUEADO");
                            $item.find('.label').removeClass('label-info');
                            $item.find('.label').addClass('label-warning');
                            $item.find('.user-js-ban').remove();
                            alertify.success("¡Bloqueado! El usuario ha sido bloqueado.");
                        }else{
                            swal("Error!", "Hubo un problema al bloquear al usuario, intentalo una vez mas.", "error");
                        }
                        jQuery('.confirm').removeClass('bookme-progress');
                    },
                    error: function (response, textStatus, errorThrown) {
                ...
                swal("¡Cancelado!", "Este usuario es seguro :)", "error");
            }
        });
