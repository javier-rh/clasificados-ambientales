...
function create_header($page_title='',$meta_desc = '',$meta_image = '',$meta_article = false)
{
    ...
    while ($info = @mysqli_fetch_array($query_result))
    {
        $id = $info['id'];
        $name = $info['asciiname'];
        $popular[$count]['tpl'] =  '<li><a class="selectme" data-id="'.$id.'" data-name="'.$name.'" data-type="'.$lang['CITY'].'"><span>'.$name.'</span></a></li>';
        $count++;
    }
    ...
    while ($info = @mysqli_fetch_array($query_result))
    {
        ...
        $name = $info['asciiname'];
        if($count == 1){
            $states[$count]['tpl'] =  '<li class="selected"><a class="selectme" data-id="'.$country_code.'" data-name="'.$lang['ALL'].' '.$countryName.'" data-type="'.$lang['COUNTRY'].'"><strong>'.$lang['ALL'].' '.$countryName.'</strong></a></li>';
        }
...
function get_the_value($link){
    ...
    if (!empty($link)) {
        ...
        return $link = str_replace(array('http://', 'https://'), '', $link);
    }
    return $link;
}
