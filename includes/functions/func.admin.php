...
function transaction_success($transaction_id)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if($info['transaction_method'] == 'Subscription'){
            ...
            mysqli_query($mysqli,"INSERT INTO `".$config['db']['pre']."upgrades` (`sub_id` ,`user_id` ,`upgrade_lasttime` ,`upgrade_expires`) VALUES ('".validate_input($subcription_id)."', '".validate_input($user_id)."', '".time()."','".validate_input($term)."')") OR error(mysqli_error($mysqli));
